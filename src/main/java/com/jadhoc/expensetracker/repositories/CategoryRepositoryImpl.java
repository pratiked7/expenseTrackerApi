...
public class CategoryRepositoryImpl implements CategoryRepository{
    ...
    private static final String SQL_FIND_ALL = "SELECT CATEGORY_ID, USER_ID, TITLE, DESCRIPTION FROM ET_CATEGORIES " +
            "WHERE USER_ID = ?";

    private static final String SQL_FIND_BY_ID = "SELECT C.CATEGORY_ID, C.USER_ID, C.TITLE, C.DESCRIPTION, " +
            "COALESCE(SUM(T.AMOUNT), 0) TOTAL_EXPENSE " +
            "FROM ET_TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES ON C.CATEGORY_ID = T.CATEGORY_ID" +
    ...
    private static final String SQL_REMOVE_BY_ID = "DELETE FROM ET_CATEGORIES WHERE CATEGORY_ID = ? AND USER_ID = ?";
